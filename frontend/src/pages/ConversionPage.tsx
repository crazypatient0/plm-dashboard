...
export default function ConversionPage() {
  ...
  const { data: countData } = useApi(() => fetchRecordsCount('conversion'), deps);
  const { data: summary } = useApi(() => fetchSummary('conversion'), deps);
  const { data: records, loading } = useApi(() => fetchLatestRecords('conversion', 100), deps);
  ...
  const columns: Column<ScrapeCurrent>[] = [
    ...
    },
    {
      ...
      width: '15%',
      render: (r) => {
        const state = (r.raw_data as Record<string, unknown>)?.state as string | null;
        return state ? (
          <span className="text-sm" style={{ color: 'var(--color-primary)' }}>{state}</span>
        ) : (
    ...
    },
    {
      ...
      width: '20%',
      render: (r) => {
        const fmt = (r.raw_data as Record<string, unknown>)?.target_format as string | null;
        return <span className="text-secondary text-sm">{fmt ?? '---'}</span>;
      },
    },
    {
      key: 'scraped_at',
